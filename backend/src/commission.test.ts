...
test("brackets: whole-order rate by bracket, split 50/50", () => {
  ...
  assert.deepEqual(calculateCommission(2_500_000), { rate: 0.03, totalCommission: 75_000, buyerShare: 37_500, sellerShare: 37_500 });
...
test("boundaries use >= : exactly 1M → 3%, exactly 10M → 5%; just below stays in the lower bracket", () => {
  assert.equal(calculateCommission(999_999).rate, 0.01);
  assert.equal(calculateCommission(1_000_000).rate, 0.03);
  assert.equal(calculateCommission(4_999_999).rate, 0.03);
